...
export default function CatalogueClient({ catalogue, products, shareUrl, waNumber }: Props) {
  ...
  const inquireWA = useCallback((p: Product) => {
    const msg = `Hi, I'm interested in *${p.name}*.\n\nCatalogue: ${shareUrl}\n\nPlease share pricing and availability.`
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-5xl mx-auto px-4 py-5">
        ...
        <div className="bg-[#0F2557] rounded-xl p-5 mb-4 flex items-start justify-between gap-4">
          <div className="min-w-0">
            {catalogue.category && (
              <div className="inline-block bg-[#F5A623] text-[#0F2557] text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide mb-2">{catalogue.category}</div>
            )}
            <h1 className="text-lg font-bold text-white mb-1 leading-snug">{catalogue.name}</h1>
            <div className="text-white/60 text-xs">
              {products.length} products{catalogue.dealExpiry ? ` · Closes ${catalogue.dealExpiry}` : ''}
            </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={p.productId} onClick={() => { setModal(p); setShowInq(false); setInqDone(false) }}
                  className="bg-white border border-gray-200 rounded-xl overflow-hidden cursor-pointer hover:border-[#0F2557] hover:-translate-y-0.5 transition-all">
                  <div className="aspect-square bg-gray-50 relative flex items-center justify-center overflow-hidden">
                    {img ? <ProductImage src={img} alt={p.name} className="w-full h-full object-cover" /> : <span className="text-4xl">📦</span>}
                    ...
                    {!p.isSoldOut && d > 0 && <div className="absolute top-2 left-2 bg-[#F5A623] text-[#0F2557] text-[10px] font-bold px-1.5 py-0.5 rounded">{d}% off</div>}
                    {p.expiryDate && <div className="absolute bottom-2 left-2 bg-red-50 border border-red-200 text-red-600 text-[9px] font-medium px-1.5 py-0.5 rounded">Exp {p.expiryDate}</div>}
                  </div>
                  <div className="p-3">
                    ...
                    {p.stockLocation && <div className="text-[10px] text-gray-400 mt-0.5">📍 {p.stockLocation}</div>}
                  </div>
                </div>
              )
            })}
        ...
        {filtered.length > 0 && grid === 'list' && (
          <div className="flex flex-col gap-2">
            {filtered.map(p => {
              ...
              return (
                <div key={p.productId} onClick={() => { setModal(p); setShowInq(false); setInqDone(false) }}
                  className="bg-white border border-gray-200 rounded-xl overflow-hidden cursor-pointer hover:border-[#0F2557] transition-all flex">
                  <div className="w-20 h-20 bg-gray-50 flex items-center justify-center shrink-0 relative overflow-hidden">
                    {img ? <ProductImage src={img} alt={p.name} className="w-full h-full object-cover" /> : <span className="text-2xl">📦</span>}
                    {!p.isSoldOut && d > 0 && <div className="absolute top-1 left-1 bg-[#F5A623] text-[#0F2557] text-[9px] font-bold px-1 py-0.5 rounded">{d}%</div>}
                  </div>
      ...
      {/* Footer */}
      <div className="text-center py-8 text-xs text-gray-400 border-t border-gray-100 mt-8">
        <div className="font-medium text-gray-500 mb-1">Surpluss — Global Inventory Exchange</div>
        <div>All deals verified · Bulk pricing · No middlemen</div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4" onClick={() => setModal(null)}>
          <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl overflow-hidden max-h-[92vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="bg-[#0F2557] px-4 py-3 flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setModal(null)} className="w-7 h-7 rounded-lg border border-white/20 bg-white/10 flex items-center justify-center text-white shrink-0 text-lg leading-none">×</button>
            </div>
            <div className="overflow-y-auto flex-1">
              <div className="aspect-video bg-gray-100 flex items-center justify-center overflow-hidden">
                {primaryImg(modal) ? <ProductImage src={primaryImg(modal)} alt={modal.name} className="w-full h-full object-cover" /> : <span className="text-6xl">📦</span>}
              </div>
              <div className="p-4">
                ...
                <div className="flex items-baseline gap-3 mb-4 pb-4 border-b border-gray-100">
                  {modal.offerPrice != null
                    ? <><span className="text-2xl font-bold text-[#0F2557]">{formatPrice(modal.offerPrice)}</span>
                      ...
                      {calcDiscount(modal.mrp, modal.offerPrice) > 0 && <span className="bg-[#F5A623] text-[#0F2557] text-xs font-bold px-2 py-0.5 rounded">{calcDiscount(modal.mrp, modal.offerPrice)}% off</span>}</>
                ...
                <div className="grid grid-cols-2 gap-px bg-gray-100 border border-gray-100 rounded-xl overflow-hidden mb-4">
                  {[
                    ...
                    <div key={d.label} className="bg-white p-3">
                      ...
                      <div className={`text-xs font-semibold ${d.label === 'Expiry' || (d.label === 'Available' && modal.isSoldOut) ? 'text-red-600' : 'text-gray-800'}`}>{d.value}</div>
                    </div>
                  ))}
                </div>
                {modal.description && <p className="text-sm text-gray-600 leading-relaxed mb-4">{modal.description}</p>}
                {allImgs(modal).length > 1 && (
                  <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
                    {allImgs(modal).map((img, i) => <ProductImage key={i} src={img} alt="" className="w-16 h-16 rounded-lg object-cover border border-gray-200 shrink-0" />)}
                  </div>
                )}
              </div>
              {showInq && !inqDone && (
                <div className="px-4 pb-4 border-t border-gray-100 pt-4">
                  <div className="text-xs font-bold text-[#0F2557] uppercase tracking-wide mb-3">Inquiry — {modal.name}</div>
                  ...
                  <button onClick={() => setInqDone(true)} className="w-full bg-[#0F2557] text-white py-2.5 rounded-lg text-sm font-semibold">Send inquiry</button>
                </div>
              )}
              {inqDone && (
                <div className="px-4 pb-4 pt-4 border-t border-gray-100 text-center">
                  <div className="text-green-500 text-3xl mb-2">✓</div>
                  <div className="text-sm font-semibold text-gray-800 mb-1">Inquiry sent</div>
                  <div className="text-xs text-gray-400">We&apos;ll contact you within 24 hours</div>
                </div>
              )}
            </div>
            {!inqDone && (
              <div className="p-4 border-t border-gray-100 flex flex-col gap-2 shrink-0">
                ...
                <button onClick={() => setShowInq(!showInq)} className="w-full flex items-center justify-center gap-2 bg-[#0F2557] text-white py-3 rounded-xl text-sm font-semibold hover:bg-[#1A3570]">
                  {showInq ? '× Close' : '✉ Send inquiry form'}
                </button>
              </div>
            )}
          </div>
