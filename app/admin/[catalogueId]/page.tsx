  ...
  if (!catalogue) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/admin" className="text-[#0F2557] text-sm mt-2 block">← Back to admin</Link>
      </div>
    </div>
  )
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="flex items-center gap-3">
            <button onClick={saveCollection} disabled={collSaving}
              ...
              {collSaving ? 'Saving…' : collSaved ? '✓ Saved' : 'Save collection details'}
            </button>
  ...
  return (
    <div className="relative group">
      ...
      <button onClick={() => inputRef.current?.click()} disabled={uploading}
        className={`w-14 h-14 rounded-lg border-2 border-dashed flex items-center justify-center overflow-hidden transition-all relative ${url ? 'border-transparent' : 'border-gray-300 hover:border-[#0F2557] bg-gray-50'}`}
        title={url ? 'Click to replace image' : `Upload image ${slot}`}>
        {url && !uploading && (
        ...
        {uploading && (
          <div className="absolute inset-0 bg-white/80 rounded-lg flex items-center justify-center">
            <div className="w-4 h-4 border-2 border-[#0F2557] border-t-transparent rounded-full animate-spin" />
          </div>
        )}
        {!url && !uploading && <span className="text-gray-400 text-xl leading-none">+</span>}
        {url && !uploading && (
      ...
      {error && (
        <div className="absolute top-full left-0 mt-1 bg-red-50 border border-red-200 text-red-600 text-[10px] rounded px-2 py-1 whitespace-nowrap z-10">{error}</div>
      )}
    </div>
  )
}
...
ENDOFFILEcat > "app/admin/[catalogueId]/page.tsx" << 'ENDOFFILE'
'use client'
import { useState, useEffect, useRef } from 'react'
import Link from 'next/link'
import type { Catalogue, Product } from '@/types'
